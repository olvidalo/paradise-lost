...
const placesPath = ('./src/data/places.csv')
...
placesData.forEach((row, index) => {
  	...
  	stringColumns.forEach(columnName => validator.assert(row[columnName].length > 1, `${placesPath} row ${index + 2}: Column "${columnName}" is empty.`))
  	...
  	coordinateColumns.forEach(columnName => {
  	   ...
  	   validator.assert(!isNaN(floatValue), `${placesPath} row ${index + 2}: ${columnName} is missing or not a valid floating point number ("${value}").`)
  	...
  	validator.assert(numMatches > 0, `${placesPath} row ${index + 2} (${row['Place']}): Could not parse passages. The "Passages" column must contain one or more strings, seperated by "/", matching the pattern: "passage text" (reference) [morality]`)
 })
